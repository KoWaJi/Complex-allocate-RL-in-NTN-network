...
from Sim_env.rlc import Rlc, RLC_BINARY_TX_ACTION
...
class UeRb:
    ...
    def get_state(self): #找出符合误码率的最小的nk，再返回d
        n_rb = 0
        if self.count == 5:
            self.count = 0
        if self.count == 1:  #每次判断完用户是否在基站内都会调用getstate，用户在首次接入和待够5个ttl时才会传
            self.channel_state = self.channel.get_state()        #CSI，而QSI是一直传的
